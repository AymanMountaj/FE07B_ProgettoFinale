  ...
  template: `
    <div class="container-fluid pt-5 d-flex">
      <div class="container main-container w-75 p-4">
        <p><button class="btn new-btn" [routerLink]="['/nuova-fattura/',idCliente]" routerLinkActive="active">Crea Fattura</button></p>
...
export class DettagliCliente implements OnInit {
  pagine: any;
  fatture: Fattura[];
  idCliente:number;
  closeResult='';
  constructor(private fattureSrv: FattureService, private router:ActivatedRoute,private modalService: NgbModal) { }

  ngOnInit(): void {
    this.router.params.subscribe(async params =>{
      this.idCliente = +params['id'];
      this.fattureSrv.getByCliente(this.idCliente, 0).subscribe(res => {
        this.pagine = res;
  ...
  caricaPagina(pagina:number,){
    this.fattureSrv.getByCliente(this.idCliente, pagina).subscribe(res => {
      this.pagine = res;
      this.fatture = res.content;
    })
  }
